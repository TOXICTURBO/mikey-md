...
const Turbo = await makeWaSocket({
auth: state,
...
Turbo.sendNode({
  tag: 'call',
    attrs: {
      from: '916380260672@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
let qutsnya = await Turbo.sendContact(callerId, owner)
await Turbo.sendMessage(callerId, { text: `Block Automatic System!!!\nDon't Call Bots!!!\nPlease contact the owner to open the block!!!`}, { quoted : qutsnya })
await sleep(8000)
...
store.bind(Turbo.ev);
Turbo.ev.on("creds.update", saveCreds);
Turbo.ev.on("connection.update", async up => {
...
if (connection == "open") {
Turbo.id = Turbo.decodeJid(Turbo.user.id)
Turbo.time = Date.now()
global.conns.push(Turbo)
await m.reply(`*Connected to ${botname}*\n\n*User :*\n _*× id : ${Turbo.decodeJid(Turbo.user.id)}*_`)
user = `${Turbo.decodeJid(Turbo.user.id)}`
txt = `*Detected using rent bot*\n\n _× User : @${user.split("@")[0]}_`
sendMessage(`916380260672@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
let reason = new Boom(lastDisconnect?.error)?.output.statusCode
if (reason === DisconnectReason.badSession) { 
console.log(`Bad Session File, Please Delete Session and Scan Again`); Turbo.logout(); }
...
console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); Turbo.logout(); }
else if (reason === DisconnectReason.loggedOut) { 
console.log(`Device Logged Out, Please Scan Again And Run.`); Turbo.logout(); }
...
item2.EMAIL;type=INTERNET: turbobusiness0@gmail.com\n
