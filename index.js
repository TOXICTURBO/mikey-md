...
require('./Turbo.js')
nocache('../Turbo.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))
require('./index.js')
nocache('../index.js', module => console.log(color('[ CHANGE ]', 'green'), color(`'${module}'`, 'green'), 'Updated'))

async function TurboBot() {
...
try{
		if (connection === 'close') {
			...
			if (reason === DisconnectReason.badSession) {
				...
				TurboBot();
			} else Turbo.end(`Unknown DisconnectReason: ${reason}|${connection}`)
		}
		...
		if (update.connection == "open" || update.receivedPendingNotifications == "true") {
            ...
            console.log(color(`🌿Connected to => ` + JSON.stringify(Turbo.user, null, 2), 'yellow'))
			await delay(1999)
            console.log(chalk.yellow(`\n\n               ${chalk.bold.blue(`[ ${botname} ]`)}\n\n`))
            console.log(color(`< ================================================== >`, 'cyan'))
	        console.log(color(`\n${themeemoji} YT CHANNEL: TurboMods`,'magenta'))
            console.log(color(`${themeemoji} GITHUB: TURBOHYPER `,'magenta'))
            console.log(color(`${themeemoji} INSTAGRAM: @toxic_turbo777 `,'magenta'))
...
Turbo.ev.on('creds.update', await saveCreds)

    // Anti Call
    Turbo.ev.on('call', async (TurboPapa) => {
    let botNumber = await Turbo.decodeJid(Turbo.user.id)
    let TurboBotNum = db.settings[botNumber].anticall
    if (!TurboBotNum) return
    console.log(TurboPapa)
    for (let TurboFucks of TurboPapa) {
    if (TurboFucks.isGroup == false) {
    if (TurboFucks.status == "offer") {
    let TurboBlokMsg = await Turbo.sendTextWithMentions(TurboFucks.from, `*${Turbo.user.name}* can't receive ${TurboFucks.isVideo ? `video` : `voice` } call. Sorry @${TurboFucks.from.split('@')[0]} you will be blocked. If accidentally please contact the owner to be unblocked !`)
    Turbo.sendContact(TurboFucks.from, global.owner, TurboBlokMsg)
    await sleep(8000)
    await Turbo.updateBlockStatus(TurboFucks.from, "block")
    }
    }
    }
    })
...
TurboWlcm = await getBuffer(ppuser)
TurboLft = await getBuffer(ppuser)
                if (anu.action == 'add') {
                const Turbobuffer = await getBuffer(ppuser)
                let TurboName = num
...
"sourceUrl": `${wagc}`}}})
                	...
                	const Turbobuffer = await getBuffer(ppuser)
                    const Turbotime = moment.tz('Asia/Kolkata').format('HH:mm:ss')
	                const Turbodate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
                	let TurboName = num
                    const Turbomembers = metadata.participants.length
                    Turbobody = `┌─❖
│「 𝗚𝗼𝗼𝗱𝗯𝘆𝗲 👋 」
└┬❖ 「 @${TurboName.split("@")[0]}  」
   │✑  𝗟𝗲𝗳𝘁 
...
const Turbodate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let TurboName = num
Turbobody = ` 𝗖𝗼𝗻𝗴𝗿𝗮𝘁𝘀🎉 @${TurboName.split("@")[0]}, you have been *promoted* to *admin* 🥳`
   Turbo.sendMessage(anu.id,
 { text: Turbobody,
 contextInfo:{
 mentionedJid:[num],
...
const Turbodate = moment.tz('Asia/Kolkata').format('DD/MM/YYYY')
let TurboName = num
Turbobody = `𝗢𝗼𝗽𝘀‼️ @${TurboName.split("@")[0]}, you have been *demoted* from *admin* 😬`
Turbo.sendMessage(anu.id,
 { text: Turbobody,
 contextInfo:{
 mentionedJid:[num],
...
console.log(err)
    ...
    Turbo.ev.on('messages.update', async chatUpdate => {
        for(const { key, update } of chatUpdate) {
			if(update.pollUpdates && key.fromMe) {
				...
				if(pollCreation) {
	                ...
	                Turbo.appenTextMessage(prefCmd, chatUpdate)
				}
			}
		}
    })
...
Turbo.sendKatalog = async (jid , title = '' , desc = '', gam , options = {}) =>{
let message = await prepareWAMessageMedia({ image: gam }, { upload: Turbo.waUploadToServer })
...
return Turbo.relayMessage(jid, tod.message, {messageId: tod.key.id})
} 

Turbo.send5ButLoc = async (jid , text = '' , footer = '', img, but = [], options = {}) =>{
...
Turbo.sendButImg = async (jid, path, teks, fke, but) => {
...
Turbo.sendMessage(jid, fjejfjjjer, { quoted: m })
}
             ...
             */
Turbo.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
                let type = await Turbo.getFile(path, true)
                ...
                let m
                try {
                    m = await Turbo.sendMessage(jid, message, { ...opt, ...options })
                } catch (e) {
                    console.error(e)
                    m = null
                } finally {
                    if (!m) m = await Turbo.sendMessage(jid, { ...message, [mtype]: file }, { ...opt, ...options })
                    file = null // releasing the memory
                    return m
                }
            }

//Turbo.sendFile = async (jid, media, options = {}) => {
        //let file = await Turbo.getFile(media)
        ...
        //else type = "document"
        //return Turbo.sendMessage(jid, { [type]: file.data, ...options }, { ...options })
      //}
...
Turbo.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
     ...
     * @returns 
     */
    Turbo.sendPoll = (jid, name = '', values = [], selectableCount = 1) => { return Turbo.sendMessage(jid, { poll: { name, values, selectableCount }}) }
